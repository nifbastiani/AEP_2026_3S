...
function adicionar_carona() {
  ...
  const caronaPath = './caronas.json'; 
  const caronaList = fetchJson(caronaPath); 
    ...
    if ((carona_data && carona_origem_local && carona_destino_local && carona_carro_placa && carona_carro_modelo) !== "" ) {
      ...
      caronaList.push(novaCarona);
      persistJson(caronaList);
...
async function persistJson(objectList) {
  try {
    const updatedJsonData = JSON.stringify(objectList, null, 2);
    localStorage.setItem('myJsonData', updatedJsonData);
  } catch (error) {
    console.error('An error occurred:', error.message);
  }
...
function consultar() {
  const tabelaCorpo = document.getElementById("tabela_corpo");
  const carona = fetchJson('https://nifbastiani.github.io/AEP_2026_3S/AEP/carona.json');
  console.log(carona);
  for (caronas in carona) { 
    const tr = document.createElement('tr');
    const content = `
    <td> ${ caronas.qtdPassageiro } </td>
    <td> ${ caronas.origem } </td>
    <td> ${ caronas.destino } </td>
    <td> ${ caronas.horarioSaida } </td>
    <td> ${ caronas.horarioChegada } </td>
    <td> ${ caronas.data } </td>
    <td> ${ caronas.preco } </td>
